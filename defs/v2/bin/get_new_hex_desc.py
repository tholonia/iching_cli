...
# Initialize OpenAI client with local API endpoint
client = OpenAI(
...
def get_contextual_response(context_files, hex_num_str):
    ...
    # Create the prompt
    prompt = f"""
    write a narrative paragraph describing the essence and archetypal meaning of this
    uploaded hexagram {hex_num_str} without any reference to tholonic concepts.

    Follow that with
    another narrative paragraph that briefly describes the tholonic concepts and how
    they apply to this hexagram {hex_num_str}. Refer to the hexagram by name, not by number.
    Use the name of the hexagram from the JSON file or uploaded documents, not the
    traditional name.

    Do NOT add titles to each paragraph, just write the paragraphs.
    Do not reference the original Chinese name for the hexagram.  For example "Contemplation (Kuan)" is incorrect,  Just write "Contemplation", and place it in quotes.
    Place the name of the hexagram in quotes.
    Write 2 paragraphs ONLY
    """

    try:
        # Create a new client for each request to ensure fresh context
