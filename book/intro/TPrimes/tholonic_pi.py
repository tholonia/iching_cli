...
CONSTANTS = {
    "pi": {
        ...
        "symbol": "π",
        "offset": 0,
        "init": {"parent": 1, "inst": 2, "limit": 5, "contrib": 3, "mult": 4},
    },
...
def calculate_next(
    ...
    if constant_type == "pi":
        ...
        iCHILD = PARENT - (1 / LIMITATION) + (1 / CONTRIBUTION)
...
def compute_tholonic_constant(constant_type, max_iter=1000000, places=5):
    ...
    for count in range(max_iter):
        ...
        if count % 100 == 0:  # Show progress every 100 iterations
            ...
            # print(
            #     f"{const['symbol']:>{col_var}} {matching_digits:<{col_res}} {str(count):<{col_count}} {elapsed_time:<{col_time}.2f}"
            # )

    # Final result
    elapsed_time = time.time() - start_time
    final_result = PARENT #* multiplier
    print(
        G + f"{const['symbol']:>{col_var}} {str(final_result):<{col_res}} {str(count):<{col_count}} {elapsed_time:<{col_time}.2f}" + X
    )
    return final_result
