...
# Annotate vertices with large numbers
for i, value in enumerate(tetra_values):
    ax.text(*tetrahedron_vertices[i], str(value), fontsize=18, color='black', ha='center', weight='bold')

for i, value in enumerate(inverted_tetra_values):
    ax.text(*inverted_tetrahedron_vertices[i], str(value), fontsize=18, color='black', ha='center', weight='bold')
...
pos = nx.spring_layout(G, seed=42, k=2.5)  # Increased k value for more spacing
...
nx.draw(G, pos,
       ...
       edge_color="gray",
       node_size=3500,  # Larger nodes to accommodate bigger text
       font_size=20,    # Much larger font for numbers
