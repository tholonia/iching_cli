...
<ul><li><B>{core['lines'][5]['position']}</B> ({core['line_type'][5]}) <I>{core['lines'][5]['name']}</I> - {core['lines'][5]['meaning']} <i>Moving line</i>: {core['lines'][5]['changing']}</li></ul>
<ul><li><B>{core['lines'][4]['position']}</B> ({core['line_type'][4]}) <I>{core['lines'][4]['name']}</I> - {core['lines'][4]['meaning']} <i>Moving line</i>: {core['lines'][4]['changing']}</li></ul>
<ul><li><B>{core['lines'][3]['position']}</B> ({core['line_type'][3]}) <I>{core['lines'][3]['name']}</I> - {core['lines'][3]['meaning']} <i>Moving line</i>: {core['lines'][3]['changing']}</li></ul>
<ul><li><B>{core['lines'][2]['position']}</B> ({core['line_type'][2]}) <I>{core['lines'][2]['name']}</I> - {core['lines'][2]['meaning']} <i>Moving line</i>: {core['lines'][2]['changing']}</li></ul>
<ul><li><B>{core['lines'][1]['position']}</B> ({core['line_type'][1]}) <I>{core['lines'][1]['name']}</I> - {core['lines'][1]['meaning']} <i>Moving line</i>: {core['lines'][1]['changing']}</li></ul>
<ul><li><B>{core['lines'][0]['position']}</B> ({core['line_type'][0]}) <I>{core['lines'][0]['name']}</I> - {core['lines'][0]['meaning']} <i>Moving line</i>: {core['lines'][0]['changing']}</li></ul>
...
def generate_markdown_from_json(json_data, sfnum):
    ...
    markdown = ""  # Remove get_yaml() from here - it should only be in the combined file
...
"""
    ...
    # Save individual hexagram file
    markdown_file = os.path.join(markdown_dir, f"hexagram_{sfnum}.md")
    try:
...
def main():
    ...
    for sfnum in HEXAGRAMS:
        ...
        # Generate markdown and add to combined output
        combined_output += "\n" + generate_markdown_from_json(json_data, sfnum)

    # Write combined output to iching.md
